    ...
    public void addActionEvent() {
        ...
        new Thread() {
            public void run() {
                ...
                GameControler.getBuiltObjects().forEach(gObj -> {
                    if (gObj.objectHasKey()) {
                        System.out.println("adding mouselistener to: " + gObj.getName());
                        JLabel gOJLabel = gObj.getObjectLabel();
                        pCont.add(gOJLabel).setVisible(true);
                        gOJLabel.addMouseListener(new MouseListener() {
                            @Override
                            public void mousePressed(MouseEvent e) {
                                if (e.getButton() == MouseEvent.BUTTON1) {
                                    if (GameControler.getGameStatus() == GameControler.RUNNING) {
                                        int[] playerCoords = GameControler.getPlayerCoords();
                                        int[] gObjCoords = { gObj.getX(), gObj.getY() };

                                        System.out.println("0 -> " + playerCoords[0] + " " + gObjCoords[0]);
                                        System.out.println("1 -> " + playerCoords[1] + " " + gObjCoords[1]);
                                        if (Math.abs(playerCoords[0] - gObjCoords[0]) < 50
                                                && Math.abs(playerCoords[1] - gObjCoords[1]) < 50) {
                                            System.out.println("Player is on " + gObj.getName());
                                            pCont.remove(gOJLabel);
                                            key.setRevealed(true);

                                            if (key.getRevealed()) {
                                                System.out.println("Key revealed");
                                                pCont.add(key.reveal(), 0);
                                            }
